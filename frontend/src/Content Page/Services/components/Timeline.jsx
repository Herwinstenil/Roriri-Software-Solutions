        ...
        <div className="max-w-6xl mx-auto" ref={timelineRef}>
          ...
          <div className="space-y-32">
            {timelineItems.map((item, index) => (
              <div
                key={index}
                className={`timeline-item relative transition-all duration-1000 ease-out ${visibleItems.has(index)
              ...
              >
                <div className="absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 top-1/2 z-20">
                  <div className={`w-16 h-16 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 shadow-lg flex items-center justify-center text-white font-bold text-xl transition-all duration-500 ${visibleItems.has(index) ? 'scale-100 rotate-0' : 'scale-75 rotate-45'
                    }`}>
                    {item.number}
                  </div>
                  <div className={`absolute inset-0 rounded-full bg-blue-400 animate-ping opacity-20 ${visibleItems.has(index) ? 'block' : 'hidden'
                    }`}></div>
                </div>

                <div className={`grid grid-cols-1 lg:grid-cols-2 gap-12 items-center ${index % 2 === 0 ? 'lg:text-right' : ''
                  }`}>
                  <div className={`space-y-6 ${index % 2 === 0 ? 'lg:order-1' : 'lg:order-2'
                    ...
                    <div className={`transition-all duration-700 delay-300 ${visibleItems.has(index) ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                      }`}>
                  ...
                  <div className={`${index % 2 === 0 ? 'lg:order-2' : 'lg:order-1'
                    } transition-all duration-1000 delay-200 ${visibleItems.has(index)
                ...
                {index < timelineItems.length - 1 && (
                  <div className="absolute left-1/2 bottom-0 transform -translate-x-1/2 translate-y-16">
                    <div className={`w-8 h-8 border-2 border-gray-300 rounded-full bg-white transition-all duration-500 ${visibleItems.has(index) ? 'animate-bounce' : ''
                      }`}>
